...
#
# ./Forecast_150.sh -d <FORECAST_DATE>
#   e.g. ./Forecast_150.sh -d 2017-03-22
#
usage() {
cat <<EOF
Usage: ./Forecast_150.sh [-d FORECAST_DATE] [-t FORECAST_TIME] [-c CONFIG_FILE] [-r ROOT_DIR] [-b DAYS_BACK] [-f]
    ...
    -c      Location of CONFIG_150.json. Default is Forecast.sh exist directory.
    ...
    --wrf-out       Path of WRF_OUTPUT directory. If this is set, then 
                    ...
                    will use respectively instead of CONFIG_150.json.
                    Otherwise using the values from CONFIG_150.json
    --wrf-rf        Path of WRF Rf(Rainfall) Directory. Otherwise using the 'RF_DIR_PATH' from CONFIG_150.json
    --wrf-kub       Path of WRF kelani-upper-basin(KUB) Directory. Otherwise using the 'KUB_DIR_PATH' from CONFIG_150.json
    --wrf-rf-grid   Path of WRF colombo(RF_GRID) Directory. Otherwise using the 'RF_GRID_DIR_PATH' from CONFIG_150.json
    --wrf-raincell  Path of WRF kelani-basin(Raincell) Directory. Otherwise using the 'RF_DIR_PATH' from CONFIG_150.json

    --hec-hms-model-dir  Path of HEC_HMS_MODEL_DIR directory. Otherwise using the 'HEC_HMS_MODEL_DIR' from CONFIG_150.json
...
INIT_DIR=$(pwd)
CONFIG_FILE=${ROOT_DIR}/CONFIG_150.json

# cd into bash script's root directory
cd ${ROOT_DIR}
echo "Current Working Directory set to -> $(pwd)"
if [ -z "$(find ${CONFIG_FILE} -name CONFIG_150.json)" ]; then
    echo "Unable to find $CONFIG_FILE file"
    exit 1
fi

HOST_ADDRESS=$(trimQuotes $(cat CONFIG_150.json | jq '.HOST_ADDRESS'))
HOST_PORT=$(cat CONFIG_150.json | jq '.HOST_PORT')
WINDOWS_HOST="$HOST_ADDRESS:$HOST_PORT"

RF_FORECASTED_DAYS=$(trimQuotes $(cat CONFIG_150.json | jq '.RF_FORECASTED_DAYS'))
RF_DIR_PATH=$(trimQuotes $(cat CONFIG_150.json | jq '.RF_DIR_PATH'))
KUB_DIR_PATH=$(trimQuotes $(cat CONFIG_150.json | jq '.KUB_DIR_PATH'))
RF_GRID_DIR_PATH=$(trimQuotes $(cat CONFIG_150.json | jq '.RF_GRID_DIR_PATH'))
FLO2D_RAINCELL_DIR_PATH=$(trimQuotes $(cat CONFIG_150.json | jq '.FLO2D_RAINCELL_DIR_PATH'))

OUTPUT_DIR=$(trimQuotes $(cat CONFIG_150.json | jq '.OUTPUT_DIR'))
STATUS_FILE=$(trimQuotes $(cat CONFIG_150.json | jq '.STATUS_FILE'))

HEC_HMS_MODEL_DIR=$(trimQuotes $(cat CONFIG_150.json | jq '.HEC_HMS_MODEL_DIR'))
HEC_HMS_DIR=$(trimQuotes $(cat CONFIG_150.json | jq '.HEC_HMS_DIR'))
HEC_DSSVUE_DIR=$(trimQuotes $(cat CONFIG_150.json | jq '.HEC_DSSVUE_DIR'))
DSS_INPUT_FILE=$(trimQuotes $(cat CONFIG_150.json | jq '.DSS_INPUT_FILE'))
DSS_OUTPUT_FILE=$(trimQuotes $(cat CONFIG_150.json | jq '.DSS_OUTPUT_FILE'))

INFLOW_DAT_FILE=$(trimQuotes $(cat CONFIG_150.json | jq '.INFLOW_DAT_FILE'))
OUTFLOW_DAT_FILE=$(trimQuotes $(cat CONFIG_150.json | jq '.OUTFLOW_DAT_FILE'))
...
bucket_date="`date -d "${forecast_date}" +'%Y_%m_%d'`"
...
main() {
    ...
    KUB_DIR_PATH=${KUB_DIR_PATH}/${bucket_date}
    FLO2D_RAINCELL_DIR_PATH=${FLO2D_RAINCELL_DIR_PATH}/${bucket_date}
	...
	# Download Raincell and Mean-ref files. Also check Rainfall data available. 
	# If all true proceed with model running.
	echo $forecast_date
	...
	if [ $result == "proceed" ];then
		...
		if [ ${forecastStatus} == 0 ]; then
	        ...
	        HEC_HMS_SCRIPT_RELATIVE_PATH_NEW = '/home/uwcc-admin/udp_150/2008_2_Events/2008_2_Events.script'
